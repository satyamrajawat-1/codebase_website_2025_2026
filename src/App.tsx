...
const ExhibitCard: React.FC<ExhibitCardProps> = ({ title, description, color, image, rotation = 0 }) => {
  return (
    <motion.div
      whileHover={{ scale: 1.05, zIndex: 10 }}
      style={{ rotate: rotation }}
      className="relative group cursor-pointer bg-black border-4 border-black rounded-2xl overflow-hidden w-[300px] h-[400px] flex-shrink-0 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]"
    >
      {/* Image Layer */}
      <img 
        src={image} 
        ...
        onError={(e) => {
          (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(title)}&background=random&color=fff&size=400`;
        }}
      />
...
const HorizontalScrollRow = ({ direction = 1, speed = 1 }: { direction?: 1 | -1, speed?: number }) => {
...
export default function App() {
  ...
  const videoScale = useTransform(scrollYProgress, [0, 0.6], [1, 4]);
  ...
  return (
    <div className="bg-black text-white font-sans selection:bg-[#ccff00] selection:text-black min-h-screen overflow-x-hidden">
      ...
      <section ref={heroRef} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden pt-20">
        ...
        {/* Scrolling Image Element - Centered behind text */}
        <motion.div 
          style={{ 
            scale: videoScale, 
            y: videoY,
          ...
          className="absolute z-5 w-[300px] md:w-[500px] h-[180px] md:h-[300px] overflow-hidden border-4 border-black rounded-[2rem] shadow-[20px_20px_0px_0px_rgba(204,255,0,1),0px_0px_60px_0px_rgba(204,255,0,0.2)]"
        >
          <img 
        ...
        <div className="container mx-auto px-6 relative z-20 text-center">
          ...
          >
            <h1 className="text-[14vw] md:text-[10vw] font-black leading-[0.85] tracking-tighter uppercase mb-8">
              IIIT Kota <br />
              <span className="text-[#ccff00]">Code</span><span className="relative">
                Base
                <motion.div 
          ...
          <motion.p 
            ...
            className="max-w-2xl mx-auto text-xl md:text-2xl text-white/70 mb-12 font-black uppercase tracking-tighter"
          >
            The Free and Open Source Society of IIIT Kota. Promoting open source development and coding culture.
          </motion.p>
          ...
          >
            <button className="bg-white text-black px-10 py-5 rounded-full font-black text-lg uppercase hover:bg-[#ccff00] transition-colors flex items-center gap-3 border-4 border-black shadow-[6px_6px_0px_0px_rgba(204,255,0,1)]">
        ...
        >
          ...
          <div className="w-[2px] h-12 bg-gradient-to-b from-[#ccff00] to-transparent rounded-full" />
        </motion.div>
      </section>

      {/* Horizontal Scroll Section */}
      <section className="py-32 bg-white text-black relative overflow-hidden border-y-8 border-black">
